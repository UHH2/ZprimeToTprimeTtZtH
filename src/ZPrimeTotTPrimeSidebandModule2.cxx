...
#include "UHH2/ZprimeToTprimeTtZtH/include/ZPrimeTotTPrimeSidebandReconstruction2.h"
...
class ZPrimeTotTPrimeSidebandModule2 : public uhh2::AnalysisModule {

 public:
  explicit ZPrimeTotTPrimeSidebandModule2(uhh2::Context&);
...
private:
  ...
  std::unique_ptr<uhh2::Selection> btag_sel;
...
ZPrimeTotTPrimeSidebandModule2::ZPrimeTotTPrimeSidebandModule2(uhh2::Context& ctx){
  ...
  TOPjet2_sel.reset(new NTopJetSelection(1,-1,TopJetId(PtEtaCut( 200., 2.4))));
  jet2_sel.reset(new NJetSelection(2, -1));

  //BTAG
  btag_sel.reset(new NBTagSelection(1, -1));
  ...
  higgstag_sel.reset(new NTopJetSelection(0, 0, higgsjetID));
  ...
  ZWtag_sel.reset(new NTopJetSelection(0, 0, ZWjetID));
  ...
  const std::string ZprimeTotTPrime_hyps_label("ZPrimeTotTPrimeSidebandReconstruction2");
  ...
  ZprimeTotTPrime_reco.reset(new ZPrimeTotTPrimeSidebandReconstruction2(ctx, NeutrinoReconstruction, ZprimeTotTPrime_hyps_label));
...
bool ZPrimeTotTPrimeSidebandModule2::process(uhh2::Event& event){
  
  if(berror)  std::cout<<"/////////////////////////////////////SelectionModule2 L:218 Am Anfang///////////////////////////////////////////////////////////////////////////////"<<std::endl;
...
/////////////////////////////////////////////////////////// Input Histogramme ///////////////////////////////////////////////////////////////////////////////
 ...
 muon_twodcut_h->fill(event);
  ...
  //clean AK4 jets from first (highest PT) AK8 jet
  ak4_cleaner->process(event);
  const bool pass_jet2 = jet2_sel->passes(event);
  if(!pass_jet2) return false;

 if(berror)  std::cout<<"SelectionModule L:294 vor Tagger"<<std::endl;  
  ////////////////////////////////////////////////////////// Tagger //////////////////////////////////////////////////////////////////////////////////
 
 bool pass_higgstag = higgstag_sel->passes(event);
...
//  event_clean_h->fill(event);
  ////////////////////////////////////////////////////////////////
 if(berror) std::cout<<"SidebandModule: vor cleaning"<<std::endl;
...
if(berror) std::cout<<"SelectionModule couting signal events " <<std::endl;
 ///////////      counting signal events      /////////////////////////
 /////AK8 und AK4 auf urspruenglich zurueck setzten

 event.topjets->clear();
 event.topjets->reserve(uncleaned_AK8->size());
 std::cout<<"vor for Schleife"<<std::endl;
 for(const auto & j : *uncleaned_AK8) event.topjets->push_back(j); 
 sort_by_pt<TopJet>(*event.topjets);

 std::cout<<"vor higgstag Selection"<<std::endl;
 pass_higgstag = higgstag_sel->passes(event);
 pass_zwtag=false;
 if(!pass_higgstag){
 std::cout<<"vor zwtag Selection"<<std::endl;
   pass_zwtag = ZWtag_sel->passes(event);
   if(!pass_zwtag) return false;
 }
 std::cout<<"vor toptag Selection"<<std::endl;

 const bool pass_toptag = toptag_sel->passes(event);

if(berror) std::cout<<"SelectionModule couting signal events: tagger cleaning "<<event.topjets->size() <<std::endl;
...
if(berror) std::cout<<"SelectionModule couting signal events: Reconstruction " <<std::endl;
 // reco_primlep->process(event);
...
if(berror) std::cout<<"SelectionModule couting signal events: Ende " <<std::endl;
...
UHH2_REGISTER_ANALYSIS_MODULE(ZPrimeTotTPrimeSidebandModule2)
