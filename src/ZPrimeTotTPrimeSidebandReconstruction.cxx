...
ZPrimeTotTPrimeSidebandReconstruction::ZPrimeTotTPrimeSidebandReconstruction(Context & ctx, const NeutrinoReconstructionMethod & neutrinofunction, const string & label): m_neutrinofunction(neutrinofunction) {
...
ZPrimeTotTPrimeSidebandReconstruction::~ZPrimeTotTPrimeSidebandReconstruction() {}



bool ZPrimeTotTPrimeSidebandReconstruction::process(uhh2::Event & event) {
...
std::vector<TopJet>* ZPrimeTotTPrimeSidebandReconstruction::AK8cleaning(std::vector<Jet>* usedAK4,uhh2::Event & event ){
...
bool ZPrimeTotTPrimeSidebandReconstruction::LepHadTop(std::vector<LorentzVector> neutrinos, const Particle & lepton , ZPrimeTotTPrimeReconstructionHypothesis hyp,uhh2::Event & event ){
  ...
  for(const auto & neutrino_p4 : neutrinos){
	...
	if(berror) std::cout << "SidebandREconstruction> L 147 cleaned topjets size "<< topjets->size()<<std::endl;
	//	if(!(topjets->size()))break;
	if(topjets->size()){
	  hyp.set_HZW_v4(topjets->at(0).v4()); 
