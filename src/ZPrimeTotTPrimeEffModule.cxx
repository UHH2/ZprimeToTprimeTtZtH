...
private:
 ...
 //BTag Hists
  ...
  std::unique_ptr<Hists> topjet_wrong_h;
  std::unique_ptr<Hists> topjet_right_h;
...
ZPrimeTotTPrimeEffModule::ZPrimeTotTPrimeEffModule(uhh2::Context& ctx){ 
  ...
  //// OBJ CLEANING
  muo_cleaner.reset(new MuonCleaner    (AndId<Muon>    (PtEtaCut  (45., 2.1), MuonIDMedium())));
  ...
  jet_sel.reset(new NJetSelection(2,-1, JetId(PtEtaCut( 45., 2.4))));         
  ...
  btag2_sel.reset(new NJetSelection(2,-1,bjetID_loose ));
 ...
 //Hist btag
  ...
  //wrong W and right W
  topjet_wrong_h.reset(new TopJetHists(ctx, "topjet_wrong"));
  topjet_right_h.reset(new TopJetHists(ctx, "topjet_right"));
...
if(berror) std::cout<<"anzahl jets und Top Jets nach AK8 cleaning "<<event.jets->size()<<" "<<event.topjets->size()<<std::endl;
  ...
  for(const Jet ak4:*event.jets){
    ...
    if(deltar_ak4hyp > 1.2) AK4Jets ->push_back(ak4);
  }
