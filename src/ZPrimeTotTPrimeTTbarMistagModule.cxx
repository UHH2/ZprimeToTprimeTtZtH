...
class ZPrimeTotTPrimeTTbarMistagModule: public uhh2::AnalysisModule {

public:
  explicit ZPrimeTotTPrimeTTbarMistagModule(uhh2::Context&);
...
private:
  ...
  std::unique_ptr<uhh2::Selection> btag1_sel;       // two btags (ak4 jets)
 ...
 //Reco Hists
  ...
  std::unique_ptr<Hists> topjet_chi2cut_before_tagger_h;
...
ZPrimeTotTPrimeTTbarMistagModule::ZPrimeTotTPrimeTTbarMistagModule(uhh2::Context& ctx){ 
 ...
 if(isMC){ 
  ...
  btag1_sel.reset(new NJetSelection(1,-1,bjetID_medium ));
  ...
  ttbar_reco.reset(new LepTopReconstruction_mistag(ctx, NeutrinoReconstruction, ttbar_hyps_label));
 ...
 //divide event.jets in reco used jets and the rest
  ...
  // const TopJetId higgsjetID = AndId<TopJet>(ZPrimeTotTPrimeHiggsTag(100,150), Tau21(1) ); 
  ZWtag_sel.reset(new NTopJetSelection(1, -1, higgsjetID));
 ...
 //Hist reco
  ...
  topjet_chi2cut_before_tagger_h.reset(new TopJetHists(ctx, "topjet_chi2cut_before_tagger"));
  ...
  input_zwtag_h->set_TopJetId(higgsjetID);
...
//   zw_notop_chi2min_btag1_h.reset(new ZPrimeTotTPrimeHypothesisHists(ctx, "zw_notop_chi2min_btag1",ZprimeTotTPrime_hyps_label,ZprimeTotTPrime_chi2_label));
 ...
 h_ZWtag = ctx.declare_event_output< std::vector<TopJet> > ("HiggsTag");
...
bool ZPrimeTotTPrimeTTbarMistagModule::process(uhh2::Event& event){
 ...
 if (!isMC){
  ...
  ////////////////////////////////////////////////////////// btag Selction //////////////////////////////////////////////////////////////////////////////////
  const bool pass_btag = btag1_sel->passes(event);
  ...
  for(const TopJet ak8:*event.topjets){
    ...
    for(const Jet ak4:*usedak4){
      ...
      if(berror) std::cout<<"SelectionModule L:858 DeltaR(ak4, ak8)<1.2 topjet is removed  "<<deltar <<"  v4  "<<ak4.v4()<<std::endl;
  ...
  AK4Jets_btagcleaned->reserve(event.jets->size()-1);
  for(auto j:*event.jets){
    if(!(deltaR(j,usedak4->at(0))==0 || deltaR(j,usedak4->at(1))==0)) AK4Jets_btagcleaned->push_back(j);
    // bool b_used=false;
    // for(auto usedjet:*usedak4){
    //   if(deltaR(j,usedjet)==0)b_used=true;
    // }
    // if(!b_used)AK4Jets_btagcleaned->push_back(j);
  }
  event.set(h_notused_jets,*AK4Jets_btagcleaned );
  
  if (berror)std::cout<<"Set new AK4 jet collection "<<std::endl;
  ...
  if(berror)std::cout << "ZPrimeTotTPrimeTTbarMistagModule: nach kin Reco"<<std::endl;
  ...
  topjet_chi2cut_before_tagger_h->fill(event);
  ...
  event.set(h_ZWtag,*topjets_higgstag );
...
UHH2_REGISTER_ANALYSIS_MODULE(ZPrimeTotTPrimeTTbarMistagModule)
